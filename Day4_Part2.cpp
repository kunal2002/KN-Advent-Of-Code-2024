...
int help_find_sequence(vector<string>& str)
{
    int n = str.size(), m = str[0].size(), ans = 0;
    for(int i = 0; i < n - 2; i++)
    {
        for(int j = 0; j < m - 2; j++)
        {
            int flag = 0;
            if(str[i][j] == 'M' && str[i + 1][j + 1] == 'A' && str[i + 2][j + 2] == 'S')
                flag = 1;
            if(str[i][j] == 'S' && str[i + 1][j + 1] == 'A' && str[i + 2][j + 2] == 'M')
                flag = 1;
            int ii = i, jj = j + 2;
            if(str[ii][jj] == 'M' && str[ii + 1][jj - 1] == 'A' && str[ii + 2][jj - 2] == 'S')
            {
                if(flag)
                    ans++;
            }
            if(str[ii][jj] == 'S' && str[ii + 1][jj - 1] == 'A' && str[ii + 2][jj - 2] == 'M')
            {
                if(flag)
                    ans++;
            }
