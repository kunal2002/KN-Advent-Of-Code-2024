...
vector<int> topoSort(vector<int>& seq, vector<int> adj[], unordered_map<int,int>& pos) 
{
    // code here
    // cout << seq << "\n";
    vector<int> indegree(101, -1);
    int n = seq.size();
    for(int i = 0; i < n; i++)
    {
        indegree[seq[i]]++;
    ...
    while(!q.empty())
    {
        ...
        {
            indegree[x]--;
            if(indegree[x] == 0 && pos[x] > 0)
                q.push(x);
        }
    }
    return topo;
}
int middle_val_finder(vector<int> adj[], vector<int>& seq, int n)
{
    unordered_map<int, int> position;
    for (int i = 0; i < n; ++i)
        position[seq[i]] = i + 1;
    // Check every edge (u -> v) in the adjacency list
    for (int u = 0; u < n; ++u) 
    {
        for (int v : adj[seq[u]]) 
        {
            if (position[seq[u]] > position[v] && position[v] != 0)
            {
                vector<int> sorted = topoSort(seq, adj, position);
                return sorted[n / 2];
            }
        }
    }
...
{  
    ...
    int ans = 0;
    for(auto x : lists)
        ans += middle_val_finder(adj, x, x.size());
    cout << ans;
}
int32_t main() 
{
