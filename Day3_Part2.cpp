...
{  
    ...
    while(getline(cin, res))
        str += res;
    // take worst case, 3 * 3 digit multiplication at the end of string
    // add m,u,l,(,x,y,z,*,x,y,z,) = 12 chars
    int n = str.size();
    rep(i, 0, 12)
        str.push_back('$');
    long long ans = 0;
    ...
    for(int i = 0; i < n; i++)
    {
        if(str[i] == 'm' && str[i + 1] == 'u' && str[i + 2] == 'l' && enable == 1)
        {
            if(str[i + 3] == '(')
            {
                string t1 = "";
                ...
                if(flag)
                    continue;
                ans += ((long long)stoi(t1) * (long long)stoi(t2));
                i = j;
            }
        }
        else if(str[i] == 'd' && str[i + 1] == 'o' && str[i + 2] == 'n' && str[i + 3] == '\'' && str[i + 4] == 't' && str[i + 5] == '(' && str[i + 6] == ')')
                enable = 0;
        else if(str[i] == 'd' && str[i + 1] == 'o' && str[i + 2] == '(' && str[i + 3] == ')')
                enable = 1;
    }
    cout << ans;
}
